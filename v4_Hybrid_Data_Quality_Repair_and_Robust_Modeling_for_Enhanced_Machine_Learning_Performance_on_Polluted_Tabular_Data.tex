...
directly across 1,142 pollution-level measurements, averaged over
the same five random seeds DQ4AI itself uses.
HybridDQR matches or beats the no-repair baseline in 69\% of
these measurements overall, and in 100\% of the clustering
measurements.
...
$\Delta_i(\mathcal{D})$ for each dimension $i$ as follows:
\begin{equation}
  \Delta_i = \rho_{\text{ref}}\!\left(\mathbf{q}'\right) -
              \rho_{\text{ref}}\!\left(\mathbf{q}\right),
\end{equation}
where $\mathbf{q}'$ is the quality profile after applying dimension
$i$'s repair operator $r_i$ once, and $\rho_{\text{ref}}$ is a
reference model performance estimator (e.g., a linear classifier
...
concrete case where a repair operator has essentially no effect
on a dimension, in a way the additive bound does not predict.
...
Our data-loading step includes two small, deliberate deviations from
the raw shipped files: Telco's \texttt{TotalCharges} column stores 11
new-customer rows as a single blank space rather than a number, which
we drop before training, bringing the dataset to exactly 7,032
samples -- matching the sample count DQ4AI itself reports, which
suggests DQ4AI's own preprocessing removed the same rows. Covertype's
sixteenth numerical feature is renamed from \texttt{x-ege.1} to
\texttt{y-ege} to match the column name actually present in the
prepared file; it is a genuine feature, not a duplicate.

\subsection{Reproducing the Original Benchmark}
\label{sec:reproduction}

Before building HybridDQR on top of DQ4AI's codebase, we first
reproduced DQ4AI's own original experiments independently, at the
same scale as the original study: the full 0.0--1.0 pollution sweep
in steps of 0.1 (11 levels) across all six quality dimensions,
averaged over the same 5 random seeds, using DQ4AI's own candidate
model catalog, for all ten original datasets.
We then compared our reproduced numbers,
level by level and model by model, against the original authors'
own published results (both sets of raw result files are included
in our repository, under \texttt{results/} and
\texttt{reproduce\_results/} in each task's directory). Averaged
over every matched numeric value, our reproduction reaches a mean
fidelity of 83.0\% for classification, 77.7\% for regression, and
80.5\% for clustering relative to the originally published numbers.
This reproduction is a separate, full-scale exercise from the
reduced-scale HybridDQR validation described in the rest of this
section: it establishes that our reuse of DQ4AI's polluters and
quality-measurement code behaves as the original benchmark intends,
before we reduce scope for HybridDQR's own, more expensive pipeline.
...
the original benchmark. Like DQ4AI, we average every measurement
over the same 5 random seeds (Section~\ref{sec:reproduction}); this
part of the original protocol is not reduced.

The one part of the scale we do reduce is \emph{how many} pollution
levels we sample per dimension. DQ4AI sweeps the pollution ratio
from 0.0 to 1.0 in steps of 0.1 (11 levels); HybridDQR's own pipeline
runs roughly a dozen model fits per level (the pipeline itself, plus
four baselines, one of which brute-forces multiple repair variants),
so reproducing the full 11-level sweep at that per-level cost, on
top of the 5-seed average, was not tractable within our project
timeline. We instead sample 3 to 5 evenly-spaced levels per
dimension, keeping the same even spacing across the 0.0--1.0 range.
For clustering's Covertype dataset specifically, we reduce this
further to 3 levels: Covertype has 44 categorical columns encoded
...
to over 330, making each level substantially more expensive to fit
than on any other dataset in this study.
...
model and report the best measured outcome. This search space
includes B2's own full-repair variant, so B4 is guaranteed to be a
true upper bound over every other baseline's action.
...
fit could dominate a plain average; we therefore summarize results
using the median across pollution levels and seeds rather than the
mean.
...
resampling and \texttt{cleanlab} for confident learning. Every
pollution level is run once per random seed, across the same 5
seeds DQ4AI itself uses (Section~\ref{sec:reproduction}), so the
medians reported in Section~\ref{sec:results} reflect variation
across both pollution levels and seeds.

Results are written incrementally to one JSON file per (task,
dataset) pair, named \texttt{hybriddqr\_results\_<task>\_<Dataset>.json}
under \texttt{hybriddqr/results/} in our repository. Each file is a
mapping from seed to quality dimension to a list of per-level
records, i.e.\ \texttt{\{seed: \{dimension: [level, ...]\}\}}. Each
level record carries the pollution configuration, the quality score
before and after any repair, the policy's decision for that
dimension, the estimated repair gain $\Delta_i$, the model selected
by RMS, HybridDQR's own metric, and all four baseline metrics
(B1--B4) for that same level -- so every number in
Table~\ref{tab:results} can be traced back to the exact level and
seed that produced it. A companion \texttt{summary.json} in the same
directory aggregates these records into the per-dataset, per-task,
and overall medians reported in Section~\ref{sec:results}.

The full implementation, all experiment scripts, and these raw
result files behind every number in Section~\ref{sec:results} are
available at our GitHub repository
\href{https://github.com/Quangdt21127150/DQ4AI.git}{DQ4AI}.
...
in Table~\ref{tab:datasets}, covering 1,142 pollution-level
measurements in total (6 dimensions $\times$ 3--5 levels $\times$
5 seeds $\times$ 9 datasets, minus a small number of degenerate
levels that no model could be fit on and were skipped, and minus
consistency for Letter, which has no categorical columns and is
therefore not a valid pollution target for that dimension).
Table~\ref{tab:results} reports the median performance of each
baseline and of HybridDQR, together with the fraction of
measurements in which HybridDQR matches or beats B1.

\begin{table}[t]
\centering
\caption{Median performance across all evaluated pollution levels
and seeds, by task. B1--B4 and HybridDQR columns report the task
metric (macro-F1 for classification, $R^2$ for regression, AMI for
clustering). The last column is the fraction of measurements where
...
\midrule
Classification (F1)   & 439 & 0.581 & 0.789 & 0.589 & 0.813 & 0.609 & 54.4\% \\
Regression ($R^2$)    & 449 & 0.483 & 0.611 & 0.791 & 0.838 & 0.819 & 65.9\% \\
Clustering (AMI)      & 254 & 0.041 & 0.088 & 0.041 & 0.170 & 0.041 & \textbf{100\%} \\
\midrule
\textbf{Overall}      & 1142 & 0.461 & 0.551 & 0.534 & 0.720 & 0.547 & 69.1\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Regression is where the hybrid policy comes closest to
the oracle.} HybridDQR's median $R^2$ (0.819) is far above B1's
(0.483) and within two points of B4's ceiling (0.838), while
matching or beating B1 on 65.9\% of the 449 regression
measurements. The policy chooses \textsc{robust} on 250 of these
measurements and \textsc{skip} on the remaining 199; in most of
the \textsc{robust} cases, the single selected model alone closes
almost all of the gap to the oracle.

\textbf{Clustering never underperforms the no-repair baseline, but
stays conservative relative to the oracle.} HybridDQR matches or
beats B1 on all 254 clustering measurements we tested, across
Bank, Covertype, and Letter -- but its median AMI (0.041) equals
B1's and B3's, and trails B4's (0.170) by a wide margin. The policy
chooses \textsc{robust} on 149 of the 254 measurements and
\textsc{skip} on the remaining 105, largely because consistency
pollution on Covertype's categorical columns turned out to be one
dimension that repair genuinely cannot fix, which
Section~\ref{sec:risk} discusses in detail. In these
\textsc{robust} cases the single top-ranked robust model rarely
matches what an oracle brute-force search over the same candidate
pool can find, even though it never does worse than doing nothing.

\textbf{Classification shows the weakest relative gain, driven by
two of its three datasets.} HybridDQR's median F1 (0.609) is only
modestly above B1's (0.581) and far below B2's blanket-repair
result (0.789), matching or beating B1 on just 54.4\% of the 439
classification measurements. This is driven mainly by Telco
(34.5\% of measurements $\geq$B1; median F1 0.708 vs.\ B1's 0.717)
and Credit (41.4\% of measurements $\geq$B1; median F1 0.657 vs.\
B1's 0.671), where the policy frequently chooses \textsc{robust}
and the selected robust model does not outperform the default
model by as much as B2's unconditional repair does; Contraceptive
shows the opposite pattern (86.6\% of measurements $\geq$B1). This
is a genuine limitation of the current severity thresholds and
robustness ranking for classification, not a result we are hiding:
at the thresholds we use, the policy under-values repair for Telco
and Credit relative to what B2 achieves by repairing
unconditionally.
...
tasks, 1,142 pollution-level measurements in total, averaged over
the same 5 seeds DQ4AI itself uses. The results are mixed but
real: HybridDQR matches or beats a no-repair baseline on 100\% of
clustering measurements and 69\% overall, comes within two points
of the oracle ceiling in regression, and shows a real gap in
classification, where the current severity thresholds under-value
repair on two of the three datasets we tested.
...
COVID-19 dataset and to DQ4AI's full 11-level pollution sweep,
both of which we had to scope out for this study to keep the
experiments tractable within the available compute time.
